...
async function getServices(): Promise<Record<string, Service>> {
  return (await kvGet<Record<string, Service>>("services")) ?? DEFAULT_SERVICES;
}
...
export async function PUT(req: NextRequest, { params }: { params: { slug: string } }) {
  if (!(await requireAuth(req)))
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const updated = (await req.json()) as Service;
  const services = await getServices();
  if (!services[params.slug])
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  services[params.slug] = { ...services[params.slug], ...updated, slug: params.slug };
